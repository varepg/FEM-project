...
def plot_temp_dist(
    ...
    draw_nodal_values_shaded_clim_cmap(a, mesh.coords, mesh.edof, title=title, clim=clim, cmap="YlOrRd")
    cfv.colorbar()
    cfv.show_and_wait()
...
def draw_quarter(u: NDArray, coords: NDArray, mesh: GripperMesh) -> None:
    magnification = 5.0
    cfv.draw_displacements(u, coords, mesh.edof,
                           mesh.dofs_per_node, mesh.el_type,
                           draw_undisplaced_mesh=True, magnfac=magnification,
                           title="Displacement")
...
def main() -> None:
    ...
    for i in range(0, max_idx+1, step_size):
        plot_temp_dist(a_transient[:,i], mesh, clim=(T_inf, T_max), title=f"Transient temperature, t = {i*dt:.2}s")
