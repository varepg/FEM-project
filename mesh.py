...
def main2():
    ...
    constitutive_matrix = {
        gripper.marker.nylon: (
            gripper.E_n/((1+nu_n) * (1-2 * nu_n))
            * np.array([
                    [1 - nu_n, nu_n, 0], 
                    [nu_n, 1 - nu_n, 0], 
                    [0, 0, 1/2*(1-2*nu_n)]
        ...
        gripper.marker.copper: (
            gripper.E_c/((1+nu_c) * (1-2 * nu_c))
            * np.array([
                    [1 - nu_c, nu_c, 0], 
                    [nu_c, 1 - nu_c, 0], 
                    [0, 0, 1/2*(1-2*nu_c)]
    ...
    for eltopo, elx, ely, el_marker in zip(mesh.edof, ex, ey, mesh.el_markers):
        Ke = cfc.plante(elx, ely, ep, constitutive_matrix[el_marker])
        K = cfc.assem(eltopo, K, Ke)
    ...
    bc, bc_val = cfu.applybc(mesh.bdofs, bc, bc_val, gripper.marker.q0_and_clamped)
    bc, bc_val = cfu.applybc(mesh.bdofs, bc, bc_val, gripper.marker.qh)
